...
func (s *SmartContract) Invoke(APIstub shim.ChaincodeStubInterface) sc.Response {
	...
	switch function {
		...
		case "accessHistoryAccess":
			return s.getHistoryForAccess(APIstub, args)
		...
		case "queryPolicyHistory":
			return s.getHistoryForPolicy(APIstub, args)
		case "addRule":
...
func (s *SmartContract) accessControl(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	objectAsBytes, _ := APIstub.GetState(args[1])
	...
	policyAsBytes, _ := APIstub.GetState(object.CID)

	if len(policyAsBytes)==0{
		return shim.Success(generateResponse("Deny"))
	}
...
func (t *SmartContract) getHistoryForAccess(stub shim.ChaincodeStubInterface, args []string) sc.Response {
